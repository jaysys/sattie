...
class DocumentService:
    ...
    def render_detail_page(self, slug: str) -> str:
        ...
        toc_items = "".join(
            f"<li><a href='#{html.escape(heading['anchor'], quote=True)}'>{html.escape(heading['text'])}</a></li>"
            for heading in document["headings"]
        ) or "<li>목차 없음</li>"
        related = "".join(
            f"<li><a href='{html.escape(item['public_url'], quote=True)}'>{html.escape(item['title'])}</a></li>"
